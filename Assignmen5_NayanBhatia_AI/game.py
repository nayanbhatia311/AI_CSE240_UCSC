...
class SnakeGame:
    ...
    def do_training(self):
        ...
        for game in range(1, self.args.NUM_TRAIN_ITER + 1):
            ...
            isAgentDead = False
            action = self.agent.agent_action(self.env.get_state(), INITIALIZATION, isAgentDead)
            while True:
                if isAgentDead:
                    break
                state,points,isAgentDead = self.env.step(action)
                if ate and points:
                    ate = not ate
                    self.agent.save_model()
                action = self.agent.agent_action(state, points, isAgentDead)
